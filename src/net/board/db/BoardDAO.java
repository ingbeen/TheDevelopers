...
public class BoardDAO {
	...
	public List<BoardBean> getBoardList(int page, int limit) {
		...
		try {
			...
			while (rs.next()) {
				...
				if (rs.getString("BOARD_SUBJECT") == null) {
					board.setBOARD_SUBJECT("제목이 없습니다");
				} else {
	...
	public boolean boardInsert(BoardBean board) {
		...
		try {
			...
			pstmt.setInt(6, num); // 그룹번호, 원글은 글번호로 한다
			pstmt.setInt(7, 0); // 들여쓰기 레벨
			pstmt.setInt(8, 0);	// 
	...
	public int boardReply(BoardBean board) {
		...
		try {
			...
			pstmt.setInt(6, re_ref); // 같은 그룹이므로 동일
			pstmt.setInt(7, re_lev); // 위 re_lev = re_lev + 1;
			pstmt.setInt(8, re_seq); // 위 re_seq = re_seq + 1
			pstmt.setInt(9, 0);
			pstmt.executeUpdate();
			return num;
		} catch (SQLException ex) {
